...
from mindie_llm.connector.request_router.layerwise.request_router_lwd import DecisionMetadata,  \
    RequestRouterLwd, DecisionType, MASTER_ID, RequestInfo, REQUEST_KEY_PREFILL, REQUEST_KEY_DECODE
...
class RequestRouterEdge(RequestRouterLwd):
    ...
    def recv_prefill(self):
        ...
        logger.info(f"[layerwiseDisaggregated] recv_prefill tcp comm finish, chunk_index_size:{chunk_index_size} "
    ...
    def check_10ms_for_next_decode(self):
        ...
        if wait_d_time_gap > 0.01:
            logger.info(f"[layerwiseDisaggregated] Force wait decode state exit, "
                f"wait time: {wait_d_time_gap * 1000} ms.")
            self.force_wait_d_time = None
    ...
    def prepare_lt_chunk_num_prefill_metadata(self, curr_dp_seq_len, chunk_num, is_p_first, request_key):
        ...
        metadata = LwdMetadata(request_key, start_or_end_layer, start_or_end_layer, end_of_generate_token, 
                              True, False, True, 0, True, 0, curr_dp_seq_len, 0, curr_dp_seq_len, False)
        metadata_list.append(metadata)

        for i in range(1, chunk_num):
            is_last_chunk = True if i == chunk_num - 1 else False
            metadata = LwdMetadata(request_key, start_or_end_layer, start_or_end_layer, False, True, True,
                                   True, 0, True, curr_dp_seq_len, curr_dp_seq_len, 0, curr_dp_seq_len, is_last_chunk)
            metadata_list.append(metadata)
    ...
    def prepare_chunk_prefill_metadata(self, prefill_chunk_policy, curr_dp_seq_len, dp_empty, is_p_first, request_key):
        ...
        for i in range(chunk_num):
            ...
            metadata = LwdMetadata(request_key, start_or_end_layer, start_or_end_layer, end_of_generate_token, True,
                                   False, False, 0, True, start_offset, end_offset, 0, curr_dp_seq_len, is_last_chunk)
            metadata_list.append(metadata)
    ...
    def prepare_chunk_prefill_metadata_queue(self, curr_dp_seq_len, dp_empty, request_key):
        ...
        p_first_metadata_list = self.prepare_chunk_prefill_metadata(prefill_chunk_policy, 
                                                                    curr_dp_seq_len, dp_empty, True, request_key)
        p_last_metadata_list = self.prepare_chunk_prefill_metadata(prefill_chunk_policy, 
                                                                   curr_dp_seq_len, dp_empty, False, request_key)

        # 上一个请求最后云一大段等于边几个小段长
        edge_eq_last_chunk_num = self.prefill_metadata_queue[-1][2].chunk_index_size \
            if self.prefill_metadata_queue else 0
        # 生成所有metadata并排序
        self.generate_send_metadata_to_queue(request_key, cloud_eq_edge_chunk_num_list=cloud_eq_edge_chunk_num_list,
        ...
        logger.info(f"[layerwiseDisaggregated] edge_eq_last_chunk_num: {edge_eq_last_chunk_num} curr_dp_seq_len: "
    ...
    def arrange_exec_stage(self, decision_type: DecisionType):
        ...
        if metadata:
            ...
            logger.info(f"[layerwiseDisaggregated] arrange exec stage decision_type:{decision_type.name} \
    ...
    def recv_decision_type(self):
        ...
        while (decision_type == DecisionType.DO_PREFILL_FIRST and \
            request_key not in self.request_map[REQUEST_KEY_PREFILL]) or \
    ...
    def prepare_prefill_request(self, request_key):
        ...
        if prefill_dp_max_seq_len > self.get_long_seq_len_min():
            ...
            prefill_first_metadata = LwdMetadata(request_key, 0, 0, False, True, False,
                                                 False, 0, False, 0, 0, 0, 0, False)
            prefill_last_metadata = LwdMetadata(request_key, 1, 1, True, True, False,
                                                False, 0, False, 0, 0, 0, 0, False)
